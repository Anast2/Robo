...
ADDR_AX_SYSTEM_DATA2 = 10  #Not specified in the table, keep an eye on this 
...
ADDR_AX_OPERATING_MODE = 19 #Not specified in the table, keep an eye on this 
...
class Ax12:
    ...
    def get_register1(self, reg_num):
        reg_data, dxl_comm_result, dxl_error = \
            Ax12.packetHandler.read1ByteTxRx(Ax12.portHandler,
                                             self.id,
                                             reg_num)
        Ax12.check_error(dxl_comm_result, dxl_error)
        return reg_data
    ...
    def get_register2(self, reg_num_low):
        reg_data, dxl_comm_result, dxl_error =\
            Ax12.packetHandler.read2ByteTxRx(Ax12.portHandler,
                                             self.id,
                                             reg_num_low)
        Ax12.check_error(dxl_comm_result, dxl_error)
        return reg_data
    ...
    def set_cw_angle_limit(self, angle_limit):
        ...
        if self.DEBUG:
            self.print_status("cw angle limit of ",
                              self.id, self.get_cw_angle_limit())
    ...
    def enable_torque(self):
        ...
        if self.DEBUG:
            print("Torque has been successfully enabled for dxl ID: %d" %
                  self.id)
    ...
    def disable_torque(self):
        ...
        if self.DEBUG:
            print("Torque has been successfully disabled for dxl ID: %d" %
                  self.id)
