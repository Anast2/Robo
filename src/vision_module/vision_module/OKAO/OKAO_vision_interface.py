...
from grayscale_image_array import GrayscaleImage
###############################################################################
...
# Output image type
output_img_type = p2def.OUT_IMG_TYPE_QVGA #p2def.OUT_IMG_TYPE_NONE
                      # OUT_IMG_TYPE_QQVGA
                      # OUT_IMG_TYPE_QVGA

# HVC camera angle setting
hvc_camera_angle = p2def.HVC_CAM_ANGLE_270
                       # HVC_CAM_ANGLE_0
                       # HVC_CAM_ANGLE_90
                       # HVC_CAM_ANGLE_180
...
def _parse_arg(argv):
    ...
    if argc == 3 or argc == 4:
        ...
        if baudrate not in p2def.AVAILABLE_BAUD:
            ...
            sys.exit()
        # Gets STB flag
        use_stb = p2def.USE_STB_OFF # Default setting is OFF
        if argc == 4 and argv[3] == "ON":
...
(portinfo, baudrate, use_stb) = "/dev/ttyACM0", 921600, "ON"
...
def get_emotions():
    ...
    if len(hvc_tracking_result.faces)>0:
        if hvc_tracking_result.faces[0].expression is not None:
            emotions = list()
            emotions.append(hvc_tracking_result.faces[0].expression.neutral)
            emotions.append(hvc_tracking_result.faces[0].expression.happiness)
            emotions.append(hvc_tracking_result.faces[0].expression.surprise)
            emotions.append(hvc_tracking_result.faces[0].expression.anger)
            emotions.append(hvc_tracking_result.faces[0].expression.sadness)
            emotions.append(hvc_tracking_result.faces[0].expression.neg_pos)
...
def detect_person():
    ...
    (res_code, stb_status) = hvc_p2_api.execute(output_img_type,\
                                              hvc_tracking_result, img)
    return len(hvc_tracking_result.faces)>0


# if __name__ == '__main__':
#     print(get_emotions(),get_image_array())
