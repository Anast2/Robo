...
def generate_launch_description():
    ...
    param_override_pkg13 = {'db_folder_path':f'{home_dir}/rooted/src/robot_memory/db/people.db',
                            'use_pc_camera':'True',
                            'camera_number':'0',
    ...
    pkg13_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            [FindPackageShare('vision_module'), '/launch/fake_vision_module_launch.py']
        ),
        launch_arguments={'db_folder_path':param_override_pkg13.get('db_folder_path'),
                          'use_pc_camera':param_override_pkg13.get('use_pc_camera'),
                          'camera_number':param_override_pkg13.get('camera_number'),
                          'camera_topic':param_override_pkg13.get('camera_topic')}.items()
    )
